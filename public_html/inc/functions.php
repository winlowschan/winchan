...
function importantstrings_load() {
	// This should fetch strings from a database table
	$query = query("SELECT * FROM `important_strings` ORDER BY `id` DESC") or error(db_error());
	$strings = array();
...
function importantstrings_delete($id) {
	$id = (int)$id;
	query("DELETE FROM `important_strings` WHERE `id` = $id") or error(db_error());
}

function importantstrings_edit($id, $text) {
	$id = (int)$id;
	$text = utf8tohtml($text);
	$query = prepare("UPDATE `important_strings` SET `text` = :text WHERE `id` = :id");
